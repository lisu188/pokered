...
- Runtime-facing provenance now covers the current map, the last successful warp, the last displayed source-backed message, and the last displayed message's source label, but broader movement/script trace hooks are still missing
...
- Extend the current `.sym` / `.map` overlay past the last-message source page into broader movement and script trace hooks.
