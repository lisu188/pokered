...
- Runtime-facing provenance now covers the current map plus the last successful warp, but broader movement/script trace hooks are still missing
...
- Extend the current `.sym` / `.map` overlay past the last-warp page into broader movement and script trace hooks.
