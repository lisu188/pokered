...
- Extend the current `.sym` / `.map` overlay past the current-map, last-map-state, last-state-gate, last-state-gate-source, last-interaction-branch, last-interaction, last-move, and last-message-source pages into broader object-state and script-state trace hooks.
