...
- The current overlay now also records the current `LAST_MAP` return anchor, the last successful warp's source/target object provenance, the last move attempt, the last confirm-based interaction, the last interaction branch/handler label, the last evaluated native state gate, the most recent displayed source-backed text label, and the corresponding source/local script label that emitted it.
...
- The overlay is still intentionally observational only and does not yet trace broader script-state transitions beyond the current gate source, object toggles beyond the current `LAST_MAP` anchor, or multi-step cutscene progress.
