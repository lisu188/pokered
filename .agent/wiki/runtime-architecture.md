...
- Current implemented behavior:
  ...
  - a runtime-facing `F7` overlay that cycles between the current map's header/object provenance, the current `LAST_MAP` return anchor, the last successful warp's source/target object provenance, the last move attempt, the last confirm-based interaction, the last interaction branch/handler provenance, the last evaluated native state gate, the last displayed source-backed text provenance, and that message's source/local script-label provenance from `pokered.sym` and `pokered.map`
...
- Runtime-facing provenance now includes current-map, last-map-state, last-warp, last-move, last-interaction, last-interaction-branch, last-state-gate, last-message, and last-message-source trace pages, but broader object-state/script-state trace hooks are still missing.
...
- Pressing `F7` now cycles the world help box between controls, current-map provenance lines, current `LAST_MAP` state lines, last-warp provenance lines, last-move trace lines, last-interaction trace lines, last-interaction-branch trace lines, last-state-gate trace lines, the most recent displayed source-backed text label, and the source/local script label that emitted it.
