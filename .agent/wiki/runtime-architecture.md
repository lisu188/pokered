...
- Current implemented behavior:
  ...
  - a runtime-facing `F7` overlay that cycles between the current map's header/object provenance, the current `LAST_MAP` return anchor, the current facing source-backed interaction target, the current facing source-backed text label, the current facing branch/handler preview, the current facing gate source/backing state, the last successful warp's source/target object provenance, the last move attempt, the last confirm-based interaction, the last interaction branch/handler provenance, the last evaluated native state gate, the exact source gate backing that native predicate, the last displayed source-backed text provenance, and that message's source/local script-label provenance from `pokered.sym` and `pokered.map`
...
- Runtime-facing provenance now includes current-map, last-map-state, live-facing-target, live-facing-text, live-facing-branch, live-facing-gate-source, last-warp, last-move, last-interaction, last-interaction-branch, last-state-gate, last-state-gate-source, last-message, and last-message-source trace pages, but broader object-state/script-state trace hooks are still missing.
...
- The current facing-text page previews the exact text label that would surface for the facing target, which makes live results like `_PalletTownGirlText`, `_RedsHouse1FTVWrongSideText`, and `_OaksLabRivalMyPokemonLooksStrongerText` visible before confirm is pressed.
- The current facing-branch page previews the current handler that would run for the facing target, including current branch labels and gate values such as `RedsHouse1FMomText.heal`, `RedsHouse1FTVText`, `OaksLabRivalText.afterChooseMon`, `GOT_STARTER=1`, and `FACING_UP=0`.
- The current facing gate-source page resolves that live facing predicate back to the source gate and backing state symbol, which makes current previews like `BIT_GOT_STARTER` / `wStatusFlags4` and `SPRITE_FACING_UP` / `wSpritePlayerStateData1FacingDirection` visible before confirm is pressed.
