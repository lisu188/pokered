...
- Current implemented behavior:
  ...
  - a runtime-facing `F7` overlay that cycles between the current map's header/object provenance, the last successful warp's source/target object provenance, and the last displayed source-backed text provenance from `pokered.sym` and `pokered.map`
...
- Runtime-facing provenance now includes last-warp and last-message trace pages, but broader movement/script trace hooks are still missing.
...
- Pressing `F7` now cycles the world help box between controls, current-map provenance lines, last-warp provenance lines, and the most recent displayed source-backed text label.
