...
- The current map-script page reports the live map's root script label, and on maps with a script-table dispatcher it also surfaces the current script-state WRAM label such as `wPalletTownCurScript` or `wOaksLabCurScript`.
