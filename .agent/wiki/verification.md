...
- Map import coverage currently verifies:
  ...
  - smoke coverage for the outdoor landing forward cases below `RedsHouse1F`, `BluesHouse`, and `OaksLab`, including passable house frontage checks via snapshot copies plus the collision-blocked `OaksLab` frontage, while preserving the current `smoke-ok: world=5 pos=2,2 steps=9` path
