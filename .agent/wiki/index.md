...
- The SDL app now has a runtime-facing provenance toggle on `F7` that cycles the help box between normal controls, the current map’s source header/object labels, the last successful warp’s source/target object provenance, the last displayed source-backed text label, and that message’s source/local script label loaded from `pokered.sym` and `pokered.map`.
...
- Runtime-facing provenance is still intentionally narrow: the overlay now covers the current map, the last successful warp, the last displayed source-backed message, and that message’s source/local script label, but not broader movement/script traces or object-state history.
