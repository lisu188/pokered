...
- The smoke path now also snapshot-checks passable lateral steps from the live interior landing tiles immediately after PalletTown re-entry into `RedsHouse1F`, `BluesHouse`, and `OaksLab`, which closes the matching runtime-facing indoor doorway movement gap without perturbing the established route.
- The smoke path now also asserts the paired-doorway span behavior on those interior landing tiles: stepping across onto the other live doorway tile exits immediately back into `PalletTown` for `RedsHouse1F`, `BluesHouse`, and `OaksLab`.
- Indoor forward movement from the PalletTown re-entry tiles is now explicit in coverage too: `RedsHouse1F` and `BluesHouse` step normally into the room, while `OaksLab`'s forward input is blocked by `OAKSLAB_OAK2` at `(5,10)` and immediately re-exits into `PalletTown` because the live entry tile is still a door warp.
- Forward movement from the paired interior doorway tiles is now explicit too: the non-entry door tile steps normally into the room or lab in `RedsHouse1F`, `BluesHouse`, and `OaksLab`, which closes the remaining forward-direction symmetry gap across the live PalletTown-connected interiors.
- The smoke path now also snapshot-checks blocked re-exits from the remaining paired interior doorway tiles, so both live door tiles per interior are runtime-covered for immediate PalletTown returns in `RedsHouse1F`, `BluesHouse`, and `OaksLab`.
- The smoke path now also snapshot-checks the paired-doorway lateral step-off that stays local, which extends the runtime-facing non-warp doorway coverage beyond the entry tiles in `RedsHouse1F`, `BluesHouse`, and `OaksLab`.
- Reverse doorway-span warps from the paired interior door tile back onto the entry tile are now explicit in both the smoke path and the native test binary for `RedsHouse1F`, `BluesHouse`, and `OaksLab`, which closes the last directional gap on the non-entry interior doorway tile.
